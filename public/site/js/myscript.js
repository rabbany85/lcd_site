    ...
    function getOrderStatus() {
        $.ajax({
              ...
              success: function(data) {
                if (data.printer_status == 0) {
                    setTimeout(function() {
                        document.getElementById("test").innerHTML = 1;
                        getOrderStatus();
                        
                    }, 1000);
                } else if (data.printer_status == 1) {
                    
                    $('#order_status').html('Your order is successful');
                    $('#order_message').html('Your order will be with you by ' + data.printer_time);
                    document.getElementById("test").innerHTML = 2;
                }  else if (data.printer_status == 2) {
                    $('#order_status').html('Order Failed');
                    $('#order_message').html('Sorry we couldn\'t deliver your order at this moment.');
